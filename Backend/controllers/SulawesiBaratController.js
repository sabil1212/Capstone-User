import SulawesiBaratMenu from "../models/SulawesiBaratModel.js";

export const createSulawesiBaratMenu = async (req, res) => {
  ...
  try {
    const newMenu = await SulawesiBaratMenu.create({
      restaurant_name,
      description,
      google_maps_link,
      rating,
...
export const getSulawesiBaratMenu = async (req, res) => {
  try {
    const menus = await SulawesiBaratMenu.findAll();
    res.json(menus);
  } catch (err) {
    console.error(err);
...
export const deleteSulawesiBaratMenu = async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await SulawesiBaratMenu.destroy({
      where: { id },
    });
...
export const editSulawesiBaratMenu = async (req, res) => {
  ...
  try {
    ...
    const menu = await SulawesiBaratMenu.findByPk(id);
