import NusaTenggaraBaratMenu from "../models/NusaTenggaraBaratModel.js";

export const createNusaTenggaraBaratMenu = async (req, res) => {
  ...
  try {
    const newMenu = await NusaTenggaraBaratMenu.create({
      restaurant_name,
      description,
      google_maps_link,
      rating,
...
export const getNusaTenggaraBaratMenu = async (req, res) => {
  try {
    const menus = await NusaTenggaraBaratMenu.findAll();
    res.json(menus);
  } catch (err) {
    console.error(err);
...
export const deleteNusaTenggaraBaratMenu = async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await NusaTenggaraBaratMenu.destroy({
      where: { id },
    });
...
export const editNusaTenggaraBaratMenu = async (req, res) => {
  ...
  try {
    ...
    const menu = await NusaTenggaraBaratMenu.findByPk(id);
