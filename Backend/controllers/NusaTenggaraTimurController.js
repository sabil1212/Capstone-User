import NusaTenggaraTimurMenu from "../models/NusaTenggaraTimurModel.js";

export const createNusaTenggaraTimurMenu = async (req, res) => {
  ...
  try {
    const newMenu = await NusaTenggaraTimurMenu.create({
      restaurant_name,
      description,
      google_maps_link,
      rating,
...
export const getNusaTenggaraTimurMenu = async (req, res) => {
  try {
    const menus = await NusaTenggaraTimurMenu.findAll();
    res.json(menus);
  } catch (err) {
    console.error(err);
...
export const deleteNusaTenggaraTimurMenu = async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await NusaTenggaraTimurMenu.destroy({
      where: { id },
    });
...
export const editNusaTenggaraTimurMenu = async (req, res) => {
  ...
  try {
    ...
    const menu = await NusaTenggaraTimurMenu.findByPk(id);
