import KalimantanTengahMenu from "../models/KalimantanTengahModel.js";

export const createKalimantanTengahMenu = async (req, res) => {
  ...
  try {
    const newMenu = await KalimantanTengahMenu.create({
      restaurant_name,
      description,
      google_maps_link,
      rating,
...
export const getKalimantanTengahMenu = async (req, res) => {
  try {
    const menus = await KalimantanTengahMenu.findAll();
    res.json(menus);
  } catch (err) {
    console.error(err);
...
export const deleteKalimantanTengahMenu = async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await KalimantanTengahMenu.destroy({
      where: { id },
    });
...
export const editKalimantanTengahMenu = async (req, res) => {
  ...
  try {
    ...
    const menu = await KalimantanTengahMenu.findByPk(id);
