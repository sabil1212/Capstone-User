import MalukuUtaraMenu from "../models/MalukuUtaraModel.js";

export const createMalukuUtaraMenu = async (req, res) => {
...
export const getMalukuUtaraMenu = async (req, res) => {
  try {
    const menus = await MalukuUtaraMenu.findAll();
    res.json(menus);
  } catch (err) {
    console.error(err);
...
export const deleteMalukuUtaraMenu = async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await MalukuUtaraMenu.destroy({
      where: { id },
    });
...
export const editMalukuUtaraMenu = async (req, res) => {
  ...
  try {
    ...
    const menu = await MalukuUtaraMenu.findByPk(id);
