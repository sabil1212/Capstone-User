import KalimantanUtaraMenu from "../models/KalimantanUtaraModel.js";

export const createKalimantanUtaraMenu = async (req, res) => {
  ...
  try {
    const newMenu = await KalimantanUtaraMenu.create({
      restaurant_name,
      description,
      google_maps_link,
      rating,
...
export const getKalimantanUtaraMenu = async (req, res) => {
  try {
    const menus = await KalimantanUtaraMenu.findAll();
    res.json(menus);
  } catch (err) {
    console.error(err);
...
export const deleteKalimantanUtaraMenu = async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await KalimantanUtaraMenu.destroy({
      where: { id },
    });
...
export const editKalimantanUtaraMenu = async (req, res) => {
  ...
  try {
    ...
    const menu = await KalimantanUtaraMenu.findByPk(id);
