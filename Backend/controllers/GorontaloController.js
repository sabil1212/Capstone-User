import GorontaloMenu from "../models/GorontaloModel.js";

export const createGorontaloMenu = async (req, res) => {
  ...
  try {
    const newMenu = await GorontaloMenu.create({
      restaurant_name,
      description,
      google_maps_link,
      rating,
...
export const getGorontaloMenu = async (req, res) => {
  try {
    const menus = await GorontaloMenu.findAll();
    res.json(menus);
  } catch (err) {
    console.error(err);
...
export const deleteGorontaloMenu = async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await GorontaloMenu.destroy({
      where: { id },
    });
...
export const editGorontaloMenu = async (req, res) => {
  ...
  try {
    ...
    const menu = await GorontaloMenu.findByPk(id);
