import KalimantanBaratMenu from "../models/KalimantanBaratModel.js";

export const createKalimantanBaratMenu = async (req, res) => {
  ...
  try {
    const newMenu = await KalimantanBaratMenu.create({
      restaurant_name,
      description,
      google_maps_link,
      rating,
...
export const getKalimantanBaratMenu = async (req, res) => {
  try {
    const menus = await KalimantanBaratMenu.findAll();
    res.json(menus);
  } catch (err) {
    console.error(err);
...
export const deleteKalimantanBaratMenu = async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await KalimantanBaratMenu.destroy({
      where: { id },
    });
...
export const editKalimantanBaratMenu = async (req, res) => {
  ...
  try {
    ...
    const menu = await KalimantanBaratMenu.findByPk(id);
