import KalimantanSelatanMenu from "../models/KalimantanSelatanModel.js";

export const createKalimantanSelatanMenu = async (req, res) => {
  ...
  try {
    const newMenu = await KalimantanSelatanMenu.create({
      restaurant_name,
      description,
      google_maps_link,
      rating,
...
export const getKalimantanSelatanMenu = async (req, res) => {
  try {
    const menus = await KalimantanSelatanMenu.findAll();
    res.json(menus);
  } catch (err) {
    console.error(err);
...
export const deleteKalimantanSelatanMenu = async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await KalimantanSelatanMenu.destroy({
      where: { id },
    });
...
export const editKalimantanSelatanMenu = async (req, res) => {
  ...
  try {
    ...
    const menu = await KalimantanSelatanMenu.findByPk(id);
