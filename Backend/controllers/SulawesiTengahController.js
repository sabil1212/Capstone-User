import SulawesiTengahMenu from "../models/SulawesiTengahModel.js";

export const createSulawesiTengahMenu = async (req, res) => {
  ...
  try {
    const newMenu = await SulawesiTengahMenu.create({
      restaurant_name,
      description,
      google_maps_link,
      rating,
...
export const getSulawesiTengahMenu = async (req, res) => {
  try {
    const menus = await SulawesiTengahMenu.findAll();
    res.json(menus);
  } catch (err) {
    console.error(err);
...
export const deleteSulawesiTengahMenu = async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await SulawesiTengahMenu.destroy({
      where: { id },
    });
...
export const editSulawesiTengahMenu = async (req, res) => {
  ...
  try {
    ...
    const menu = await SulawesiTengahMenu.findByPk(id);
