import SulawesiTenggaraMenu from "../models/SulawesiTenggaraModel.js";

export const createSulawesiTenggaraMenu = async (req, res) => {
  ...
  try {
    const newMenu = await SulawesiTenggaraMenu.create({
      restaurant_name,
      description,
      google_maps_link,
      rating,
...
export const getSulawesiTenggaraMenu = async (req, res) => {
  try {
    const menus = await SulawesiTenggaraMenu.findAll();
    res.json(menus);
  } catch (err) {
    console.error(err);
...
export const deleteSulawesiTenggaraMenu = async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await SulawesiTenggaraMenu.destroy({
      where: { id },
    });
...
export const editSulawesiTenggaraMenu = async (req, res) => {
  ...
  try {
    ...
    const menu = await SulawesiTenggaraMenu.findByPk(id);
