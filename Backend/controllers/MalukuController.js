import MalukuMenu from "../models/MalukuModel.js";

export const createMalukuMenu = async (req, res) => {
...
export const getMalukuMenu = async (req, res) => {
  try {
    const menus = await MalukuMenu.findAll();
    res.json(menus);
  } catch (err) {
    console.error(err);
...
export const deleteMalukuMenu = async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await MalukuMenu.destroy({
      where: { id },
    });
...
export const editMalukuMenu = async (req, res) => {
  ...
  try {
    ...
    const menu = await MalukuMenu.findByPk(id);
