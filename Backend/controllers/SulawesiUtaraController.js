import SulawesiUtaraMenu from "../models/SulawesiUtaraModel.js";

export const createSulawesiUtaraMenu = async (req, res) => {
  ...
  try {
    const newMenu = await SulawesiUtaraMenu.create({
      restaurant_name,
      description,
      google_maps_link,
      rating,
...
export const getSulawesiUtaraMenu = async (req, res) => {
  try {
    const menus = await SulawesiUtaraMenu.findAll();
    res.json(menus);
  } catch (err) {
    console.error(err);
...
export const deleteSulawesiUtaraMenu = async (req, res) => {
  const { id } = req.params;

  try {
    const deleted = await SulawesiUtaraMenu.destroy({
      where: { id },
    });
...
export const editSulawesiUtaraMenu = async (req, res) => {
  ...
  try {
    ...
    const menu = await SulawesiUtaraMenu.findByPk(id);
